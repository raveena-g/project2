...
const Vegan_fr = () => {
    ...
    <div className="alllunch">
      <Sidebar/>
        ...
        <p1><BackButton/></p1>
    <div className="lunch">
    <h2>{locale.vegan_recipes}</h2>
        <ul>
        {data.sort(sortMethods[sortState].method).filter((recipe) => {if(recipe.vegan==locale.vegan){return recipe}}).map((recipe) => (
